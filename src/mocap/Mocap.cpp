...
void Mocap::SetServerAddress(const std::string& addr) { _server_address = addr; }

const sServerDescription& Mocap::GetServerDescription() const { return _serverDescription; }
  ...
  if (retCode != ErrorCode_OK) {
    ...
    if (verbose_) {
      ...
      printf("Application: %s (ver. %d.%d.%d.%d)\n", _serverDescription.szHostApp,
             _serverDescription.HostAppVersion[0], _serverDescription.HostAppVersion[1],
             _serverDescription.HostAppVersion[2], _serverDescription.HostAppVersion[3]);
...
float Mocap::GetFramerate() const {
  void* pResult;
  int nBytes = 0;
  ErrorCode ret = ErrorCode_OK;
...
int Mocap::Run() {
  ...
  _pClient->SetFrameReceivedCallback(
      DataHandler, this);  // this function will receive data from the server
  ...
  while (const int c = getchar()) {
    switch (c) {
      ...
      case 'c':  // connect
        ...
        break;
      case 'd':  // disconnect
        // note: applies to unicast connections only - indicates to Motive to stop sending
        // packets to that client endpoint
...
void NATNET_CALLCONV DataHandler(sFrameOfMocapData* data, void* pUserData) {
  ...
  //   (TransmitTimestamp)
  //
  // This figure may appear slightly higher than the "software latency" reported in the
  // Motive user interface, because it additionally includes the time spent preparing to
  // stream the data via NatNet.
  ...
  const double softwareLatencyMillisec =
      ...
      static_cast<double>(serverDescription.HighResClockFrequency);

  // Transit latency is defined as the span of time between Motive transmitting the frame of
  // data, and its reception by the client (now). The SecondsSinceHostTimestamp method
  // relies on NatNetClient's internal clock synchronization with the server using
  ...
  // Only recent versions of the Motive software in combination with ethernet camera systems
  // support system latency measurement. If it's unavailable (for example, with USB camera
  // systems, or during playback), this field will be zero.
  const bool bSystemLatencyAvailable = data->CameraMidExposureTimestamp != 0;

  if (bSystemLatencyAvailable) {
    // System latency here is defined as the span of time between:
    //   a) The midpoint of the camera exposure window, and therefore the average age of the
    //   photons (CameraMidExposureTimestamp)
    // and
    //   b) The time immediately prior to the NatNet frame being transmitted over the
    //   network (TransmitTimestamp)
    ...
    // Client latency is defined as the sum of system latency and the transit time taken to
    // relay the data to the NatNet client. This is the all-inclusive measurement (photons
    ...
    // You could equivalently do the following (not accounting for time elapsed since we
